...
def show_prizes_page():
    ...
    with col1:
        ...
        main_league_text = "<br>".join([f"<b>{ordinal(row['position'])}:</b> £{row['amount']}" for _, row in main_league.iterrows()])
    ...
    with col2:
        ...
        h2h_league_text = "<br>".join([f"<b>{ordinal(row['position'])}:</b> £{row['amount']}" for _, row in h2h_league.iterrows()])
    ...
    with col2:
        ...
        weekly_league_text = "<br>".join([f"£{row['amount']}" for _, row in weekly_league.iterrows()])
